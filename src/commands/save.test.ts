...
import { createTempDir, cleanupTempDir, initGitRepo, readFile } from "../test-utils";
...
describe("save command", () => {
  ...
  test("saves files to template directory", async () => {
    ...
    const templateAgents = await readFile(join(configDir, "templates", "test-save", "AGENTS.md"));
    const templateClaude = await readFile(join(configDir, "templates", "test-save", "CLAUDE.md"));
  ...
  test("throws error when no template configured", async () => {
    ...
    await expect(save({ silent: true })).rejects.toThrow("No template configured");
  });
  
  test("throws error when not in git repo", async () => {
    process.chdir(tempDir);
    
    await expect(save({ silent: true })).rejects.toThrow("Not in a git repository");
  });
  ...
  test("skips files that don't exist", async () => {
    ...
    const configDir = process.env.AGENCY_CONFIG_DIR!;
    const templateAgents = await readFile(join(configDir, "templates", "test-partial", "AGENTS.md"));
    expect(templateAgents).toBe("# Only agents");
    
    // CLAUDE.md should still be default
    const templateClaude = await readFile(join(configDir, "templates", "test-partial", "CLAUDE.md"));
