...
`agency context`. OpenCode and Pi execution units launched by `agency work` are
rooted in the authoritative writable checkout so their project and Git status
reflect the implementation repository. Orchestration remains rooted in its
...
to use it for implementation, and add its supported skill directories; this
does not make other checkout-local configuration authoritative. Agents must
follow the authority reported by `agency context`.
