...
describe("makePrBranchName", () => {
  test("replaces %branch% placeholder with branch name", () => {
    expect(makePrBranchName("feature-foo", "%branch%--PR")).toBe("feature-foo--PR");
    expect(makePrBranchName("feature-foo", "PR/%branch%")).toBe("PR/feature-foo");
...
describe("extractSourceBranch", () => {
  describe("with %branch% placeholder", () => {
    test("extracts source branch from PR branch name", () => {
      expect(extractSourceBranch("feature-foo--PR", "%branch%--PR")).toBe("feature-foo");
      expect(extractSourceBranch("PR/feature-foo", "PR/%branch%")).toBe("feature-foo");
    ...
    test("handles complex patterns", () => {
      expect(extractSourceBranch("pr-feature-foo-ready", "pr-%branch%-ready")).toBe("feature-foo");
      expect(extractSourceBranch("PR/feature/foo/ready", "PR/%branch%/ready")).toBe("feature/foo");
    });
  });
